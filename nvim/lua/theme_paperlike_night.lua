...
local function validate_contrast()
  ...
  for _, check in ipairs(checks) do
    ...
    if ratio < min_contrast then
      vim.notify(
        string.format("Warning: %s contrast ratio %.1f:1 (below WCAG AA %.1f:1)", 
        check.name, ratio, min_contrast),
        vim.log.levels.WARN
      )
    end
  end
...
local function hl(group, opts)
  ...
  -- Skip empty groups
  if not opts.fg and not opts.bg and not opts.bold and not opts.italic 
    and not opts.underline and not opts.undercurl then
    return
  end
