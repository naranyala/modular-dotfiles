      ...
      map({ "i", "n" }, "<C-d>", function()
        ...
        if sel then
          ...
          action_state.get_current_picker(prompt_bufnr):refresh(
            finders.new_table({ results = refresh_bookmarks(), entry_maker = function(e) return e end }),
            { reset_prompt = false }
          )
        end
      end)
...
local function open_telescope_remove()
  if #M.bookmarks == 0 then return end
...
function M.setup()
  ...
  vim.keymap.set("n", "<leader>ma", add_current_line, { desc = "Bookmarks: Add current line" })
  vim.keymap.set("n", "<leader>mr", open_telescope_remove, { desc = "Bookmarks: Remove picker" })
