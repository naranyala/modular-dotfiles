...
const cli = meow(`
  ...
  Examples
    ...
    $ font-resolver https://fonts.google.com/specimen/Roboto -o ./assets
...
/**
 * Extract font name from input (could be direct name, Fontsource URL, or Google Fonts URL)
 */
function extractFontName(input: string): string {
  ...
  // If it's a Google Fonts URL (e.g., https://fonts.google.com/specimen/Roboto)
  if (input.startsWith('https://fonts.google.com/specimen/')) {
    try {
        const url = new URL(input);
        const pathParts = url.pathname.split('/');

        // Find the font name part (usually the last segment after '/specimen/')
        const specimenIndex = pathParts.indexOf('specimen') + 1;
        if (specimenIndex > 0 && specimenIndex < pathParts.length) {
          // The font name is the segment right after 'specimen'
          return pathParts[specimenIndex];
        }
    } catch (e) {
        // Fall through to treat it as a plain name if URL parsing fails unexpectedly
    }
  }
